<# 
FileName:	GLFHCFindConfigChanges.ps1
Purpose:	This script will look at each of the SQL Server instances in the server list file
			(JustServerNameList.txt), create a listing of configuation changes that were made.
			
	Date			Author						DESCRIPTION
==============		======		==========================================================
04/24/2024			peb					Original Version
...
$FilePath = "C:\Output"
$OutFile = Join-Path -path $FilePath -childPath ("GLFHCFindConfigChanges_" + $isodate + ".csv")
$servers = 'c:\Code\JustServerNameList.txt'                       # list of your SQL Servers
$Subj = "GLFHC SQL Server Configuration Changes on $NewReportDate from $ThisServer."
...
$sql = "
xp_readerrorlog 0,1, N'Configuration option'
"
...
ELSE
{
	...
	$emailTo = "wyang@glfhc.org"
	$Subject = $subj 
	$Body = "No Configuration Changes were found in the SQL Error Log files...  No action is required."
	$smtpServer = "smtpmail.glfhc.local"
	#[System.Net.ServicePointManager]::ServerCertificateValidationCallback = { return $true }
	$SMTPMessage = New-Object System.Net.Mail.MailMessage($EmailFrom, $EmailTo, $Subject, $Body) 
<#
