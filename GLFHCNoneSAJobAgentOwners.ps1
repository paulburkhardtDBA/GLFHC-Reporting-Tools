<# 
FileName:	GLFHCNoneSAJobAgentOwners.ps1
...
$FilePath = "C:\Output"
$OutFile = Join-Path -path $FilePath -childPath ("GLFHCNoneSAJobAgentOwners_" + $isodate + ".csv")
$servers = 'c:\Code\JustServerNameList.txt'                       # list of your SQL Servers
$Subj = "GLFHC SQL Server Jobs Not owned by SA Report on $NewReportDate from $ThisServer."
...
$sql = "
SELECT @@SERVERNAME Server,sj.[name], sp.[name] JobOwner 
FROM msdb.dbo.sysjobs sj
  INNER JOIN sys.server_principals sp ON sj.owner_sid = sp.[sid]
WHERE sp.[name] NOT IN (N'sa'); 
"
...
IF ((Get-Content -Path $OutFile).length -ne 0)
{
	...
	$Subject = $subj 
	$Body = "Please review and correct SQL job ownership..."
